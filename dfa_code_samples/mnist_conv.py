...
l0 = Convolution(input_shape=[batch_size, 28, 28, 1], filter_sizes=[3, 3, 1, 32], init=args.init, activation=act, bias=args.bias, name='conv1')
l1 = FeedbackConv(size=[batch_size, 28, 28, 32], num_classes=10, sparse=args.sparse, rank=args.rank, name='conv1_fb')

l2 = Convolution(input_shape=[batch_size, 28, 28, 32], filter_sizes=[3, 3, 32, 64], init=args.init, activation=act, bias=args.bias, name='conv2')
l3 = MaxPool(size=[batch_size, 28, 28, 64], ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")
l4 = FeedbackConv(size=[batch_size, 14, 14, 64], num_classes=10, sparse=args.sparse, rank=args.rank, name='conv2_fb')

l5 = ConvToFullyConnected(input_shape=[14, 14, 64])

l6 = FullyConnected(input_shape=14*14*64, size=128, init=args.init, activation=act, bias=args.bias, name='fc1')
l7 = Dropout(rate=dropout_rate)
l8 = FeedbackFC(size=[14*14*64, 128], num_classes=10, sparse=args.sparse, rank=args.rank, name='fc1_fb')

l9 = FullyConnected(input_shape=128, size=10, init=args.init, bias=args.bias, name='fc2')
...
plt.plot(range(0,args.epochs,1),test_accs)
